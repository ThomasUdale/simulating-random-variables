library('MASS')
library('progress')
library(parallel)
library(ggplot2)
library(patchwork)
library(reshape2)
library(mvtnorm)



p <- function(X,beta) {
  z <- exp(X %*% beta)
  z/(1+z)
}


log_likelihood_full <- function(beta){
  z <- X %*% t(beta)
  return(sum(z*y - log(1+exp(z))) - sum((beta)^2/(2*C*10)))
}


log_likelihood <- function(beta,c){
  z <- X[c((m/C*(c-1)+1):(m/C*c)),] %*% t(beta)
  return(sum(z*y[c((m/C*(c-1)+1):(m/C*c))] - log(1+exp(z))) - sum((beta)^2/(2*C*10)))
}

grad_log_likelihood <- function(beta,c){
  t(X[c((m/C*(c-1)+1):(m/C*c)),]) %*% (y[c((m/C*(c-1)+1):(m/C*c))] - p(X[c((m/C*(c-1)+1):(m/C*c)),],beta))
}

log_reg_posterior <- function(X,y,ss,burn,C) {
  kappa = y-0.5
  true_post <- c()
  omega <- rep(0,nrow(X))
  beta <- c(1,1)
  pb <- progress_bar$new(
    format = "sampling [:bar] :current/:total (:percent) eta: :eta, elapsed: :elapsed",
    clear = FALSE, total = (ss+burn), width = 80)
  for (i in 1:(ss+burn)) {
    pb$tick()
    omega <- BayesLogit::rpg(nrow(X),1,beta[1] + beta[2]*x)
    var <-  solve(t(X) %*% diag(omega) %*% X + solve(diag(rep(C*10,2))))
    mu <- var %*% (t(X) %*% kappa)
    beta <- mvrnorm(n = 1, mu, var)
    if (i > burn) {
      true_post <- rbind(true_post,beta)
    }
  }
  pb$terminate()
  true_post
}

sub_post <- function(c) {
  log_reg_posterior(X[c((m/C*(c-1)+1):(m/C*c)),],y[c((m/C*(c-1)+1):(m/C*c))],ss,burn,C)
}

mcmc <- function(burn_mcmc,ss_mcmc){
  pb <- progress_bar$new(
    format = "sampling [:bar] :current/:total (:percent) eta: :eta, elapsed: :elapsed",
    clear = FALSE, total = (ss_mcmc+burn_mcmc), width = 80)
  y <- c(0,0)
  j <- sample.int(ss,C,replace=T)
  xc <- do.call(cbind, lapply(c(1:C),function(c){sub_post_sample[j[c],,c]}))
  mean_xc <- rowMeans(xc)
  sd_xc <- exp(-apply(xc,1,sd))
  pi_y <- exp(sum(unlist(lapply(c(1:C),log_likelihood,beta=t(y)))))
  out <- c()
  for (t in 1:(burn_mcmc+ss_mcmc)){
    pb$tick()
    j <- sample.int(ss,C,replace=T)
    new_xc <- do.call(cbind, lapply(c(1:C),function(c){sub_post_sample[j[c],,c]}))
    sd_new_xc <- exp(-apply(new_xc,1,sd))
    mean_new_xc <- rowMeans(new_xc)
    
    new_y <-rmvnorm(1,mean_new_xc*sd_new_xc+y*(1-sd_new_xc),diag((1-sd_new_xc)))
    pi_new_y <- exp(sum(unlist(lapply(c(1:C),log_likelihood,beta=new_y))))
    
    r <- pi_new_y/pi_y * dmvnorm(y,mean_xc*sd_xc+new_y*(1-sd_xc),diag((1-sd_xc)))/dmvnorm(new_y,mean_new_xc*sd_new_xc+y*(1-sd_new_xc),diag((1-sd_new_xc)))
    if (runif(1)<min(r,1)){
      xc <- new_xc
      mean_xc <- mean_new_xc
      sd_xc <- sd_new_xc
      y <- new_y
      pi_y <- pi_new_y
    }
    if (t>burn_mcmc){
      out <- rbind(out,y)
    }
  }
  pb$terminate()
  out
}

calc_iad <- function() {
  iad <- 0
  for (var_id in c(1:2)){
    iad <- iad + sum(abs(density(out[,var_id],weights=out[,3]/sum(out[,3]),from=-10,to=10,n=2000)$y - density(full_post_sample[,var_id],from=-10,to=10,n=2000)$y))/2000*20
  }
  iad/2
}

C = 40

true_beta = c(-4,-2)

ss = 10000
burn = 2000

# m = 1000
# x <- rnorm(m,0.7,1)
# X <- cbind(rep(1,m),x)
# y <- rbinom(m,1,p(X,true_beta))
# print(sum(y))
# 
# full_post_sample <- log_reg_posterior(X,y,ss,burn,1)
# sub_post_sample <- simplify2array(pbmcapply::pbmclapply(c(1:C),sub_post,mc.cores=8))
# dimnames(sub_post_sample) <- list(c(1:ss),c(1:2),c(1:C))
# sub_post_sample_df <- melt(sub_post_sample)
# 
# 
out <- mcmc(1e4,1e4)
  
plot1 <- ggplot()+
  xlim(-8,1)+
  geom_density(data=subset(sub_post_sample_df,Var2==1),aes(x=value,group=Var3),col='blue',alpha=0.05)+
  geom_density(aes(x=full_post_sample[,1]),col='black',alpha=0.8)+
  geom_density(aes(x=out[,1]),col='red',alpha=0.4)


plot2 <- ggplot()+
  xlim(-4,3)+
  geom_density(data=subset(sub_post_sample_df,Var2==2),aes(x=value,group=Var3),col='blue',alpha=0.05)+
  geom_density(aes(x=full_post_sample[,2]),col='black',alpha=0.8)+
  geom_density(aes(x=out[,2]),col='red',alpha=0.4)
  
print(plot1+plot2)
